...
public abstract class GamePanel extends JPanel {
    ...
    public GamePanel() {
        ...
        /** Для отрисовки ходов используем graphics 2D**/
    public void paintComponent(Graphics g) {
        ...
        for (Position lm : _lastMove) {
            g.setColor(new Color(211, 198, 110, 128));
        ...
        g.setColor(new Color(255, 234, 0, 192));
        if (_selected != null) {
    ...
    private void drawBoard() {
        ...
        g.setColor(Color.decode("#f3f6f4"));
        g.fillRect(0, 0, 8 * SQUARE_SIZE, 8 * SQUARE_SIZE);

        g.setColor(Color.decode("#ddb371"));
        for (int i = 0; i < 8; i++) {
    ...
    public void loadImages() {
        try {
            String source = "/chess/resources/pieces_pictures/";
